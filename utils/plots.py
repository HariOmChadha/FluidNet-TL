...
def plot_results(path, save_folder):
    with open("results/cond/ANN_scratch/results.yaml", 'r') as f:
        results = yaml.load(f, Loader=yaml.FullLoader)

    s_std = np.std(np.array(results['scratch_srcc']), axis=0)
    m_std = np.std(np.array(results['scratch_mae']), axis=0)
    s_avg = np.mean(np.array(results['scratch_srcc']), axis=0)
    m_avg = np.mean(np.array(results['scratch_mae']), axis=0)

    s_std_2 = np.std(np.array(results['Transfer_srcc']), axis=0)
    m_std_2 = np.std(np.array(results['Transfer_mae']), axis=0)
    s_avg_2 = np.mean(np.array(results['Transfer_srcc']), axis=0)
    m_avg_2 = np.mean(np.array(results['Transfer_mae']), axis=0)

    axis = results['axis']

    main_folder = 'save_folder'
    for i in range (5):
        plot_srcc_MAE(s_avg[:, i], s_avg_2[:, i], s_std[:, i], s_std_2[:, i], axis, f'{i+1}', main_folder, tag2='srcc', tag3 = 'avg')
        plot_srcc_MAE(m_avg[:, i], m_avg_2[:, i], m_std[:, i], m_std_2[:, i], axis, f'{i+1}', main_folder, tag2='mae', tag3 = 'avg')